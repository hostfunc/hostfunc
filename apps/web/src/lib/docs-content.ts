...
export async function main(input: { targetSlug: string; payload?: Record<string, unknown> }) {
  ...
  "/docs/triggers": {
    ...
    highlights: [
      "HTTP triggers default to `requireAuth: true`; callers use a workspace API token unless the request is a verified nested `executeFunction` call.",
    ...
    guideSections: [
      {
        ...
        bullets: [
          "http: `{ requireAuth: boolean }` (defaults to true for new functions)",
