...
import { Copy, Terminal, Zap, Lock, BarChart3, Hexagon, ArrowRight, Activity, Code, Globe, Infinity } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#050505] text-slate-200 selection:bg-primary/30 selection:text-primary overflow-hidden">
      ...
      <header className="relative z-50 border-b border-white/5 bg-background/50 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className="flex items-center gap-6">
            <Link href="/docs" className="text-sm font-medium text-muted-foreground hover:text-white transition-colors">
              Docs
            </Link>
            {isPending ? (
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-6 pt-32 pb-24 md:pt-48 md:pb-32">
        ...
        >
          ...
          <motion.h1 variants={FADE_DOWN} className="text-balance text-5xl font-extrabold tracking-tight sm:text-7xl lg:text-8xl bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60 drop-shadow-sm max-w-4xl">
            The cloud for tiny composable functions.
          </motion.h1>
          
          <motion.p variants={FADE_UP} className="mt-8 text-balance text-lg sm:text-xl text-muted-foreground max-w-2xl leading-relaxed">
            Write a TypeScript function. Deploy in seconds. Seamlessly trigger workloads via HTTP, cron intervals, or direct AI bindings with zero configuration.
          </motion.p>
          
          <motion.div variants={FADE_UP} className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
            <Button asChild size="lg" className="w-full sm:w-auto h-14 px-8 text-base rounded-full shadow-[0_0_40px_rgba(var(--primary),0.3)] hover:shadow-[0_0_60px_rgba(var(--primary),0.5)] transition-all bg-primary text-primary-foreground">
              <Link href={session ? "/dashboard" : "/login"}>Start building <ArrowRight className="ml-2 w-4 h-4" /></Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="w-full sm:w-auto h-14 px-8 text-base rounded-full border-white/10 hover:bg-white/5 text-white bg-transparent backdrop-blur-md">
          ...
          <div className="relative rounded-2xl bg-[#0d1117] border border-white/10 shadow-2xl overflow-hidden">
            ...
            <div className="p-6 overflow-x-auto text-sm font-mono leading-relaxed">
              <pre className="text-gray-300">
                <code>
                  <span className="text-[#ff7b72]">import</span> fn, {'{'} secret {'}'} <span className="text-[#ff7b72]">from</span> <span className="text-[#a5d6ff]">"@hostfunc/fn"</span>;<br/>
                  <span className="text-[#ff7b72]">import</span> Stripe <span className="text-[#ff7b72]">from</span> <span className="text-[#a5d6ff]">'stripe'</span>;<br/><br/>
                  <span className="text-[#ff7b72]">export async function</span> <span className="text-[#d2a8ff]">main</span>(req: Request) {'{'}<br/>
                  <span className="text-[#8b949e]">  // Validate webhooks effortlessly</span><br/>
                  <span className="text-[#ff7b72]">  const</span> payload = <span className="text-[#ff7b72]">await</span> req.<span className="text-[#d2a8ff]">json</span>();<br/>
                  <span className="text-[#ff7b72]">  const</span> apiKey = <span className="text-[#d2a8ff]">secret</span>(<span className="text-[#a5d6ff]">'STRIPE_KEY'</span>);<br/>
                  <br/>
                  <span className="text-[#ff7b72]">  if</span> (payload.type === <span className="text-[#a5d6ff]">"payment.succeeded"</span>) {'{'}<br/>
                  {'    '}console.<span className="text-[#79c0ff]">log</span>(<span className="text-[#a5d6ff]">"Processing payment!"</span>);<br/>
                  {'  }'}<br/>
                  <br/>
                  <span className="text-[#ff7b72]">  return</span> {'{'} received: <span className="text-[#79c0ff]">true</span> {'}'};<br/>
                  {'}'}
                </code>
              </pre>
            </div>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-6 py-24 md:py-32 border-t border-white/5">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-white mb-4">Infrastructure without the overhead</h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Everything you need to run reliable serverless workloads in production right out of the box.</p>
        </div>
      ...
      <footer className="relative z-10 border-t border-white/5 bg-black/50 py-12 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-semibold text-white">hostfunc © {new Date().getFullYear()}</span>
          </div>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="group rounded-2xl border border-white/10 bg-white/[0.02] p-8 transition-all hover:bg-white/[0.04] hover:border-white/20 relative overflow-hidden backdrop-blur-sm">
      ...
      <h3 className="mb-2 text-xl font-semibold text-white group-hover:text-primary transition-colors">{title}</h3>
