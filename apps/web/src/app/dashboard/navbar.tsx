...
import { Plus, Activity, Layers, Settings, ChevronDown, User, LogOut, Hexagon, Zap } from "lucide-react";
...
export function DashboardNavbar({ user }: { user: { id: string; email: string; name?: string | null } }) {
    ...
    <header className="border-b border-border bg-background/50 backdrop-blur-md sticky top-0 z-50">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-6">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center gap-2 pl-2 pr-3 px-2 h-9 py-1" size="sm">
                ...
                <span className="text-muted-foreground text-xs bg-muted px-1.5 py-0.5 rounded-sm">/</span >
                <span className="font-semibold text-sm">hostfunc</span>
                <ChevronDown className="w-3 h-3 text-muted-foreground ml-1" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-[200px]">
              <DropdownMenuLabel className="text-xs text-muted-foreground font-normal">Active Workspace</DropdownMenuLabel>
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-8 w-8 rounded-full border border-border bg-muted/20 outline-none focus-visible:ring-1">
            ...
            <DropdownMenuContent className="w-56" align="end" forceMount>
              <DropdownMenuLabel className="font-normal border-b pb-3 mb-2">
                <div className="flex flex-col space-y-1">
                  <p className="text-sm font-medium leading-none">{user.name || "System Administrator"}</p>
