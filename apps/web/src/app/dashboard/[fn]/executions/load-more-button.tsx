      ...
      {rows.length > 0 ? (
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-ink-elevated)]/55">
          {rows.map((row) => (
            <div
              key={row.id}
              className="flex items-center justify-between border-t border-[var(--color-border)] px-4 py-3 text-sm first:border-t-0"
            >
              <div className="font-mono text-xs text-[var(--color-bone-faint)]">
                {row.triggerKind}
              </div>
              <div>{row.status}</div>
