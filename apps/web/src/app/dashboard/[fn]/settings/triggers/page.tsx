...
import { Link2, Clock, Mail, ChevronRight, Activity, Copy, CheckCircle2 } from "lucide-react";
import { useState } from "react";

export default function TriggersFunctionSettingsPage({ params }: { params: Promise<{ fn: string }> }) {
  const { fn } = use(params);
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-10">
      ...
      <div className="border-b border-white/10 pb-6">
        ...
        <p className="text-muted-foreground mt-2 max-w-xl leading-relaxed">
          Configure how your serverless code is executed. You can map functions to public web endpoints, bind them to strict scheduler crons (via Inngest), or even connect them to inbound email ingestion parsers.
        </p>
      </div>
        ...
        <div className="rounded-2xl border border-indigo-500/30 bg-[#0a0a0a] relative overflow-hidden shadow-[0_0_30px_rgba(99,102,241,0.05)]">
          ...
          <div className="p-6">
               ...
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   <h4 className="text-lg font-semibold text-white tracking-tight flex items-center gap-2">
                     HTTP Endpoint 
                     <Badge variant="secondary" className="font-mono text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-widest px-2 py-0.5">Online</Badge>
                   </h4>
                   <p className="text-sm text-muted-foreground mt-0.5">Invoke via standard web requests or APIs.</p>
                 </div>
               </div>
            ...
            <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground font-medium">
               <span>Authentication: <strong className="text-slate-300">Public (None)</strong></span>
               <span className="w-1 h-1 rounded-full bg-white/20" />
               <span>Methods: <strong className="text-slate-300">GET, POST, OPTIONS</strong></span>
            </div>
          </div>
        </div>
          ...
          <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="text-lg font-semibold text-white tracking-tight flex items-center gap-2">
                  Cron Scheduler 
                  <Badge variant="outline" className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest px-2 py-0.5">Inactive</Badge>
                </h4>
                <p className="text-sm text-muted-foreground mt-1 max-w-sm leading-relaxed">
                  Automatically dispatch this function periodically using our distributed task queue. Perfect for syncing data or cleanup jobs.
                </p>
              </div>
            </div>
          ...
          <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1 max-w-sm leading-relaxed">
                  Bind this function to listen to specific inbound emails. We parse the headers and attachments and pass it directly to your code payload.
                </p>
