...
import { KvError, kvDelete } from "@/server/kv-store";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const deleted = await kvDelete(auth.payload.fnId, body.key);
    ...
    return Response.json(
      { error: "kv_delete_failed", detail: error instanceof Error ? error.message : "unknown" },
      { status: 502 },
    );
  }
}
