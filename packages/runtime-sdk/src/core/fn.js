...
async function resolveSlug(controlPlane, token, slug) {
    if (!controlPlane || !token)
        return null;
    try {
        const res = await fetch(`${controlPlane}/api/internal/resolve?slug=${encodeURIComponent(slug)}`, { headers: { authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(5000) });
        if (!res.ok)
            return null;
...
export const fn = {
    async executeFunction(slug, input, options) {
        ...
        const timeoutMs = Math.max(1, Math.min(Number(options?.timeoutMs ?? DEFAULT_EXECUTE_TIMEOUT_MS), 300_000));
        let res;
        try {
        ...
        if (!res.ok) {
            ...
            let childJson = null;
            try {
                childJson = text ? JSON.parse(text) : null;
            }
            catch {
                childJson = null;
            }
